...
fun main() {
    ...
    println(list.filterNot { it % 2 == 0 }) // 식의 맞는 요소를 제외한 요소 골라내기
    println(listWithNull.filterNotNull()) // 널을 제외

    // filerIndexed : 인덱스와 함께 추출
    println("filerIndexed : " + list.filterIndexed { idx, value -> idx != 1 && value % 2 == 0 })
    // filerIndexedTo : 추출 후 가변형 컬렉션으로 변환
    val mutList = list.filterIndexedTo(mutableListOf()){idx, value -> idx!= 1 && value % 2 == 0}
    println("filerIndexedTo : $mutList")
    ...
    // filterIsInstance : 여러 자료형의 요소 중 원하는 자려형을 골라냄
    ...
    println(list.take(2)) // 앞 두 요소를 반환
    println(list.takeLast(2)) // 마지막 두 요소를 반환
    ...
    // 각 요소는 componentN과 대응하기 때문에 이것을 사용해 요소 반환 가능
    // N은 인덱스 번호가 아니라 1 부터 시작하는 요소의 순서 번호 ex) 요소가 5개라면 1부터 5까지
    println("component1 : " + list.component1()) // 첫 번째 요소인 1 반환

    // distinct : 중복 요소는 하나로 취급해 리스트로 반환
